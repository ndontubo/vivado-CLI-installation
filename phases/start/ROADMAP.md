...
- [ ] `--storage-path` flag: if internal free space is short, offer to put
      the VM image on an external SSD instead of hard-failing.
- [ ] Check RAM (16GB+ recommended)
- [ ] Check/install QEMU via Homebrew
- [ ] Verify Virtualization.framework + Rosetta availability
- [ ] Clear, actionable error messages for every failure case (this is a
      ...
      don't tell you *why* something failed). Disk-short message should
      state exactly how much is free, how much the chosen pairing needs,
      that installer space is reclaimable, and suggest `--storage-path`.
